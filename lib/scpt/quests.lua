...
function quest_towneltalk(Ind, msg)
	...
	if player.pclass == CLASS_WARRIOR or player.pclass == CLASS_PALADIN or player.pclass == CLASS_MIMIC or player.pclass == CLASS_ROGUE or admin then
		if player.num_blow < 2 then
			msg_print(Ind, "\255UFor a "..msg.." you attack too slowly. Your number of blows per round (BpR) should at least be 2!")
			...
			if player.pclass == CLASS_ROGUE or admin then
				msg_print(Ind, "\255UAs a rogue a third way is to dual-wield, by equipping two weapons at once!")
			end
		else
			msg_print(Ind, "\255UYou seem to be doing fine, "..msg..".")
		end
	else
		msg_print(Ind, "\255UYou seem to be doing fine, "..msg..".")
	end
