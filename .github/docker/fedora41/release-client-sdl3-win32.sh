...
STAGE_DIR="${RELEASE_STAGE_BASE}/client-sdl3-win32"
ROOT_NAME="tomenet-${RELEASE_VERSION}-client-sdl3-win32"
ROOT_DIR="${STAGE_DIR}/${ROOT_NAME}"
ARCHIVE_PATH="${RELEASE_OUTDIR}/tomenet-${RELEASE_VERSION}-client-sdl3-win32.zip"

# Build without wine, it's faster.
HAVE_WINE=no make -C "$SRC_DIR" -f makefile.sdl3 clean tomenet.exe
