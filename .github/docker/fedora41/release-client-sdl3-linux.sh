...
STAGE_DIR="${RELEASE_STAGE_BASE}/client-sdl3-linux"
ROOT_NAME="tomenet-${RELEASE_VERSION}-client-sdl3-linux-${RELEASE_ARCH}"
ROOT_DIR="${STAGE_DIR}/${ROOT_NAME}"
ARCHIVE_PATH="${RELEASE_OUTDIR}/tomenet-${RELEASE_VERSION}-client-sdl3-linux-${RELEASE_ARCH}.tar.bz2"

USE_GCU_LINUX=yes make -C "$SRC_DIR" -f makefile.sdl3 clean tomenet
