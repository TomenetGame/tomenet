...
resolve_linux_dlopen_dep() {
	local source_lib="$1"
	local dep_name="$2"
	local source_dir
	local candidate

	source_dir="$(dirname "$source_lib")"
	for candidate in \
		"${source_dir}/${dep_name}" \
		"/usr/lib64/${dep_name}" \
		"/lib64/${dep_name}" \
...
release_init() {
	...
	RELEASE_STAGE_BASE="${RELEASE_STAGE_BASE:-${RELEASE_WORKSPACE}/fedora43/.release-stage}"
	RELEASE_OUTDIR="${RELEASE_OUTDIR:-${RELEASE_WORKSPACE}}/fedora43"
...
copy_linux_deps_recursive() {
	...
	while :; do
		...
		# Fedora 43 ships libSDL2 as sdl2-compat, which dlopens libSDL3 at runtime.
		# ldd does not see that edge, so add SDL3 explicitly when bundling that SDL2 shim.
		if [[ "$base" == "libSDL2-2.0.so.0" ]] && grep -Fq 'libSDL3.so.0' < <(strings "$lib"); then
			candidate="$(resolve_linux_dlopen_dep "$lib" "libSDL3.so.0" || true)"
			if [[ -n "$candidate" ]]; then
				echo "$candidate" >> "$deps_file"
			else
				echo "warning: missing runtime dependency libSDL3.so.0 for $lib" >&2
			fi
		fi
