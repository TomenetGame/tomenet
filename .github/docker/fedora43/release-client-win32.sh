...
STAGE_DIR="${RELEASE_STAGE_BASE}/client-win32"
ROOT_NAME="tomenet-${RELEASE_VERSION}-client-win32"
ROOT_DIR="${STAGE_DIR}/${ROOT_NAME}"
ARCHIVE_PATH="${RELEASE_OUTDIR}/tomenet-${RELEASE_VERSION}-client-win32.zip"

make -C "$SRC_DIR" -f makefile.mingw clean tomenet.exe
...
cp -a "${REPO_ROOT}/TomeNET.ini.default" "${ROOT_DIR}/TomeNET.ini.default"
