 ...
 */
static int getlevel_wild(struct worldpos *wpos) {
	wilderness_type *w_ptr = &wild_info[wpos->wy][wpos->wx];
...
static bool should_we_bleed(struct worldpos *wpos, char dir) {
	...
	if (w_ptr->type != wild_info[neigh_y][neigh_x].type) {
		/* determine whether to bleed or not */
		Rand_value = seed_town + (getlevel_wild(wpos) + getlevel_wild(&neighbor)) * (93754);
		tmp = rand_int(2);

		if (tmp && (getlevel_wild(wpos) < getlevel_wild(&neighbor))) return(TRUE);
		else if (!tmp && (getlevel_wild(wpos) > getlevel_wild(&neighbor))) return(TRUE);
		else return(FALSE);
	}
				...
				if (in_bounds_wild(neighbor.wy, neighbor.wx)) {
					...
					if (should_we_bleed(&neighbor, c)) {
						...
						if (wild_info[neighbor.wy][neighbor.wx].type == w_ptr->type) {
							/* share a point */
							/* seed the number generator */
							Rand_value = seed_town + (getlevel_wild(wpos) + getlevel_wild(&neighbor)) * (89791);
