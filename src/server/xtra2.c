...
void player_death(int Ind) {
	...
	if ((p_ptr->global_event_temp & PEVF_SAFEDUN_00) && p_ptr->csane >= 0 && in_sector00_dun(&p_ptr->wpos)) {
...
//		msg_format(Ind, "Gain=%d, skill=%d", gain, skill_trauma);
	...
	if (m_ptr->hp < 0) {
		...
		if (necro) {
			...
			long gain, gain_mp, skill; /* let's make it more complicated - gain HP and MP now - C. Blue */
...
#else
			gain_mp = ((gain_mp + 1) * (gain_mp + 1)) / 75;
			if (!gain_mp) gain_mp = 1; /* level 0 monsters give energy */
#endif
