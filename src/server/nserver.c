...
//		msg_print(NumPlayers, "\377R<< Welcome to TomeNET! >>");
	...
	if (p_ptr->audio_mus && p_ptr->audio_mus < __audio_mus_max) msg_print(NumPlayers, "\374\377D --- Warning: Your music pack is outdated! ---");
#endif
	...
	if (strlen(choice) == 0) { /* we have entered an account name */
		...
		if (connp->pass && (l_acc = GetAccount(connp->nick, connp->pass, FALSE))) {
			...
			*/
			...
			/* Abuse flags #3 for actual char limit, so it's no longer hardcoded in the client */
			sflags3 |= (MAX_CHARS_PER_ACCOUNT & 0xFF);
