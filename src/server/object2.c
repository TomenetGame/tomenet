...
#if 0 /* experimentally moved to the bottom! - 2009-07-28 - C. Blue */
...
#if 0
	/* might add am again? hm (compare to artifact_flag_cost() maybe!) */
...
s64b object_value_real(int Ind, object_type *o_ptr)
{
	...
	if (o_ptr->name1)
	{
		...
		{
#if 1 /* use dedicated artifact pricing function - C. Blue */
			return(artifact_value_real(Ind, o_ptr));
#else /* just use ego pricing routines (with artifact-price-cutting) */
...
#if 0 /* zero influence for now */
			/* Special attack (exploding arrow) */
			if (o_ptr->pval != 0) {
				if (o_ptr->name1 != ART_RANDART) value *= 4;
				else value *= 1;
			}
#endif
...
#if 0 /* disabled this redundant check since it now actually misses the WINNERS_ONLY trueart exception! (see below) - C. Blue */
...
void apply_magic(struct worldpos *wpos, object_type *o_ptr, int lev, bool okay, bool good, bool great, bool verygreat, u32b resf)
{
	...
	/* Hack for possible randarts, to be created in next for loop:
	   ...
	   That means, it must be applied beforehand already.
...
//	else if (o_ptr->name2)
	if (o_ptr->name2 && !o_ptr->name1)
	{
		...
		/* Extract the other fields */
//		o_ptr->pval += a_ptr->pval;
		if (!is_magic_device(o_ptr->tval)) /* don't kill charges on EGO (of plenty) devices! */
...
void determine_level_req(int level, object_type *o_ptr)
{
	...
	if (o_ptr->tval == TV_CHEST) {
		o_ptr->level = (base / 2 + level) / 2;	/* chest level is base for calculating the item level,
							so it must be like a dungeon level - C. Blue
							(base = k_info level / 2, level = dungeonlevel usually) */
		return;
	}
	...
	/* Hack (only used in lua_determine_level_req(), which is only invoked manually):
	   -9999 means: use unmodified base value and don't use randomizer: */
	if (level == -9999) {
		i = 0;
		j = base;
	} else { /* this is what is used in-game for all purpose: */
		i = level - base;
		j = (((i * (i > 0 ? 2 : 1)) / 12 + base) * rand_range(95,105)) / 100;/* was 1:2 / 4 */
	}
...
//	}
	...
	/* potions that mustn't be transferred, otherwise resulting in 1 out-of-line char */
	if ((o_ptr->tval == TV_POTION) && (
	    (o_ptr->sval == SV_POTION_EXPERIENCE) ||
	    (o_ptr->sval == SV_POTION_LEARNING) ||
	    (o_ptr->sval == SV_POTION_INVULNERABILITY)))
		o_ptr->level = 0;

	/* scroll lower limits */
	if (o_ptr->tval == TV_SCROLL) {
		switch (o_ptr->sval) {
		case SV_SCROLL_TRAP_CREATION: if (o_ptr->level < 20) o_ptr->level = 20;
			break;
		case SV_SCROLL_FIRE:
		case SV_SCROLL_ICE:
		case SV_SCROLL_CHAOS: if (o_ptr->level < 30) o_ptr->level = 30;
			break;
		}
	}
...
#if 0
		if (!k_idx) {
//s_printf("k_idx abort.");
			return;
		}
//		if (!k_idx) break; /* always give a quest reward item */
#else
...
//	int        y, x, i, d;
	...
	for (; num > 0; --num)
	{
#if 0 /* works, but only if there's an empty floor tile */
...
#if 0 /* annoying if you have auto-id and want to quickly /dis loot */
	/* auto-pickup inscription for ammo */
	if (is_ammo(o_ptr->tval) && object_known_p(Ind, o_ptr))
	{
		if (o_ptr->sval != SV_AMMO_MAGIC)
			o_ptr->note = quark_add("!=LM");
		return;
	}
#endif
}
