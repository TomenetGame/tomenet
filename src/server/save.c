...
/* DEBUGGING PURPOSES - the_sandman */
	...
	wr_s32b(o_ptr->id);
	wr_s32b(o_ptr->find_id);
	wr_string(o_ptr->find_name);
	wr_s32b(o_ptr->find_turn);
	wr_u32b((u32b)(o_ptr->find_time & 0xFFFFFFFF));
	if (sizeof(time_t) >= 8) wr_u32b((u32b)(((u64b)o_ptr->find_time) >> 32));
	else wr_u32b(0);
	wr_s16b(o_ptr->find_wpos.wx);
	wr_s16b(o_ptr->find_wpos.wy);
	wr_s16b(o_ptr->find_wpos.wz);
	wr_byte((unsigned char)o_ptr->find_dun);
	wr_byte(o_ptr->is_mushroom); //pointless, see load2.c comment
