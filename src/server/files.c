...
*/
 ...
 #ifndef ALT_EXPRATIO
		adv_prev = ((s64b)(*(p_ptr->player_exp))[(is_admin(p_ptr) ? PY_MAX_LEVEL : PY_MAX_PLAYER_LEVEL) - 2] * (s64b)p_ptr->expfact / 100L);
 #else
		adv_prev = (s64b)(*(p_ptr->player_exp))[(is_admin(p_ptr) ? PY_MAX_LEVEL : PY_MAX_PLAYER_LEVEL) - 2];
 #endif
		adv_exp_prev = (s32b)(adv_prev);
	else {
		s64b adv_prev = 0;
 #ifndef ALT_EXPRATIO
		s64b adv = ((s64b)(*(p_ptr->player_exp))[p_ptr->lev - 1] * (s64b)p_ptr->expfact / 100L);
		if (p_ptr->lev > 1) adv_prev = ((s64b)(*(p_ptr->player_exp))[p_ptr->lev - 2] * (s64b)p_ptr->expfact / 100L);
 #else
		s64b adv = (s64b)(*(p_ptr->player_exp))[p_ptr->lev - 1];
		if (p_ptr->lev > 1) adv_prev = (s64b)(*(p_ptr->player_exp))[p_ptr->lev - 2];
 #endif
		adv_exp = (s32b)(adv);
		adv_exp_prev = (s32b)(adv_prev);
	}
...
bool process_player_name(int Ind, bool sf) {
	...
	if (strlen(p_ptr->name) < CNAME_MIN_LEN || strlen(p_ptr->name) < ACC_CHAR_MIN_LEN) { /* (null terminator char at the end) */
		...
		if (min_char == min_acc)
			Destroy_connection(p_ptr->conn, format("Your account and character names must be at least %d characters long!", min_char));
		else
			Destroy_connection(p_ptr->conn, format("Your character name must be at least %d characters long!", min_char));
