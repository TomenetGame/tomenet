...
/* Simple los() version for placement of static light sources, added for Kurzel's adventure module,
   specifically the blazing fire feat which inspired the idea to make it actually blazing,
   but also in retrospective for light-emitting questors ;). - C. Blue */
bool los_zcave(cave_type **zcave, int y1, int x1, int y2, int x2) {
	/* Delta */
	int dx, dy;
	/* Absolute */
	int ax, ay;
...
/* For new Ents' "greenery-based monster sensing" ability:
   Grids are connected by plant life, aka 'Line of Greenery'? We even allow dirt and mud to count for now...
   NOTE: The monster must be y1,x1 and the player y2,x2! */
#define is_greenery(zcave, y, x) \
    (zcave[y][x].feat == FEAT_GRASS || zcave[y][x].feat == FEAT_BUSH || zcave[y][x].feat == FEAT_TREE  || zcave[y][x].feat == FEAT_DIRT || zcave[y][x].feat == FEAT_MUD || feat_is_water(zcave[y][x].feat))
#define is_greenery_transmissible(zcave, y, x) \
    (zcave[y][x].feat == FEAT_BUSH || zcave[y][x].feat == FEAT_TREE || cave_los(zcave, y, x))
bool logreenery(struct worldpos *wpos, int y1, int x1, int y2, int x2) {
	/* Delta */
	int dx, dy;
	/* Absolute */
	int ax, ay;
	...
	/* Destination, ie monster we as Ent want to sense, must stand on greenery to cause plant-based effects (rustling sound, air movement...) */
	if (!is_greenery(zcave, y1, x1)) return(FALSE);
	...
	if (!dx) {
		...
		if (dy > 0) {
			for (ty = y1 + 1; ty < y2; ty++)
				if (!is_greenery_transmissible(zcave, ty, x1)) return(FALSE);
		/* North -- check for walls */
		} else {
			for (ty = y1 - 1; ty > y2; ty--)
				if (!is_greenery_transmissible(zcave, ty, x1)) return(FALSE);
	...
	if (!dy) {
		...
		if (dx > 0) {
			for (tx = x1 + 1; tx < x2; tx++)
				if (!is_greenery_transmissible(zcave, y1, tx)) return(FALSE);
		/* West -- check for walls */
		} else {
			for (tx = x1 - 1; tx > x2; tx--)
				if (!is_greenery_transmissible(zcave, y1, tx)) return(FALSE);
	...
	if (ax == 1) {
		if (ay == 2) {
			if (is_greenery_transmissible(zcave, y1 + sy, x1)) return(TRUE);
		}
	}
	/* Horizontal "knights" */
	else if (ay == 1) {
		if (ax == 2) {
			if (is_greenery_transmissible(zcave, y1, x1 + sx)) return(TRUE);
		}
	}
	...
	if (ax >= ay) {
		...
		/* the LOS exactly meets the corner of a tile. */
		while (x2 - tx) {
			if (!is_greenery_transmissible(zcave, ty, tx)) return(FALSE);
...
/* Get both, attr and char, for a monster race, to display to the player on the main screen.
   ...
   'm_ptr' is NULL when getting a player's mimicry form visuals instead of a real monster. */
static void get_monster_visual(int Ind, monster_type *m_ptr, monster_race *r_ptr, int m_idx, byte *ap, char32_t *cp) {
	player_type *p_ptr = Players[Ind];
	byte a;
	char32_t c;
	...
	if (m_ptr) {
		/* normal monster, potentially custom 'attr' mapping: */
		if (!m_ptr->special && !m_ptr->questor && p_ptr->custom_mapping)
			a = p_ptr->r_attr[m_ptr->r_idx];
		/* 1) player golem/questor: r_ptr is actually already m_ptr->r_ptr;
		   2) no custom mapping, or out-of-the-ordinary monster we cannot currently handle in custom mappings: */
		else a = r_ptr->d_attr;

		/* normal monster, potentially custom 'char' mapping: */
		if (!m_ptr->special && !m_ptr->questor && p_ptr->custom_mapping
		    && !(((p_ptr->ascii_uniques && (r_ptr->flags1 & RF1_UNIQUE)) || p_ptr->ascii_monsters)))
			c = p_ptr->r_char[m_ptr->r_idx];
		/* player golem: r_ptr is actually already m_ptr->r_ptr: */
		else if (m_ptr->special && r_ptr->d_char == 'g' && m_ptr->r_idx)
		...
		/* questor: r_ptr is actually already m_ptr->r_ptr: */
		else if (m_ptr->questor
		...
		/* no custom mapping, or out-of-the-ordinary monster we cannot currently handle in custom mappings: */
		else c = r_ptr->d_char;
	} else {
		/* Default r_info.txt attr/char mapping: */
		a = r_ptr->d_attr;
		c = r_ptr->d_char;
	}
	...
	if (((*w_ptr & CAVE_MARK) || (viewable_any && !p_ptr->blind)) || (p_ptr->admin_dm)) {
		...
		if ((feat == FEAT_MORE) || (feat == FEAT_WAY_MORE) ||
			...
			*ap_back = *ap;
			...
			if (!is_admin(p_ptr) && d_ptr && !(d_ptr->known & DKF_SEEN)
...
void cave_illuminate_rad(struct worldpos *wpos, cave_type **zcave, s16b x, s16b y, int rad, u32b flags) {
	...
	CAVE_ILLUM_MACRO(zcave, x, y, x, y + 2, flags)
	CAVE_ILLUM_MACRO(zcave, x, y, x + 1, y + 2, flags)
	CAVE_ILLUM_MACRO(zcave, x, y, x - 1, y + 2, flags)
	...
	CAVE_ILLUM_MACRO(zcave, x, y, x + 2, y, flags)
	CAVE_ILLUM_MACRO(zcave, x, y, x + 2, y + 1, flags)
	CAVE_ILLUM_MACRO(zcave, x, y, x + 2, y - 1, flags)
	...
	CAVE_ILLUM_MACRO(zcave, x, y, x + 2, y + 2, flags)
	CAVE_ILLUM_MACRO(zcave, x, y, x - 2, y + 2, flags)
	CAVE_ILLUM_MACRO(zcave, x, y, x + 2, y - 2, flags)
	CAVE_ILLUM_MACRO(zcave, x, y, x - 2, y - 2, flags)
	...
	for (yy = min_y; yy <= max_y; yy++) {
		for (xx = min_x; xx <= max_x; xx++) {
			...
			CAVE_ILLUM_MACRO(zcave, x, y, xx, yy, flags)
		}
	}
}
...
/*	    (wind_gust > 0 ? 1 : (wind_gust < 0 ? 2 : 0))
	    ...
	    (wind_gust > 0 ? 2 * (season == SEASON_WINTER ? WEATHER_SPEED_SNOW : WEATHER_SPEED_NORMAL) * WEATHER_GEN_TICKS_NORMRAIN
	    : (wind_gust < 0 ? 2 * (season == SEASON_WINTER ? WEATHER_SPEED_SNOW : WEATHER_SPEED_NORMAL) * WEATHER_GEN_TICKS : 0)),
	    WEATHER_GEN_TICKS,
	    (season == SEASON_WINTER) ? WEATHER_DENSITY_NORMAL : WEATHER_DENSITY_THICK,
	    (season == SEASON_WINTER) ? WEATHER_SPEED_SNOW * WEATHER_GEN_TICKS :
	    /* hack: for non-windy rainfall, accelerate raindrop falling speed by 1: */
	    (wind_gust ? WEATHER_SPEED_NORMAL * WEATHER_GEN_TICKS : WEATHER_GEN_TICKS_NORMRAIN),
