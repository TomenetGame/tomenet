...
int turn_plus = 1;		/* How fast turns progress. [1 = Normal Operation, anything else is hazardous, undefined behaviour] */
int turn_plus_extra = 0;	/* Extra dungeon() calls done in the scheduler for each normal dungeon() call. [0 = Normal operation, anything else is hazardous, undefined behaviour] */
 ...
 */
u16b max_r_idx = 0;
u16b max_rur_idx = 0; /* Unique monsters eligible for winner-respawning */
 ...
 */
char priv_note[MAX_NOTES][MSG_LEN], priv_note_sender[MAX_NOTES][NAME_LEN], priv_note_target[MAX_NOTES][NAME_LEN], priv_note_u[MAX_NOTES][MSG_LEN], priv_note_date[MAX_NOTES][NAME_LEN];//actually 11 is fitting instead of NAME_LEN, but w/e we avoid hardcoding at least =p
...
u32b hack_sigil_f[7] = { 0 }; //same as ego_granted_flags
