...
//#endif
	...
	if (!can_go_down(wpos, 0x1) && !can_go_up(wpos, 0x1)) ; /* special or what? */
	else if (!can_go_down(wpos, 0x1) && can_go_up(wpos, 0x1)) place_up_stairs(wpos, y, x);
	else if (!can_go_up(wpos, 0x1) && can_go_down(wpos, 0x1)) place_down_stairs(wpos, y, x);
	else if (rand_int(100) < 50) place_down_stairs(wpos, y, x);
	else place_up_stairs(wpos, y, x);
}
 ...
 */
bool vault_aux_aquatic(int r_idx) {
			...
			for (y = y1; y <= y2; y++)
				for (x = x1; x <= x2; x++)
					if (!rand_int(5)) {
						...
						drop_near(TRUE, 0, &forge, 0, wpos, y, x);
					}
 ...
 */
bool vault_aux_jelly(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 */
bool vault_aux_animal(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 * Helper function for "monster nest (undead)"
 */
bool vault_aux_undead(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
...
bool vault_aux_chapel(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
...
bool vault_aux_lesser_chapel(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 */
bool vault_aux_kennel(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
...
bool vault_aux_lesser_kennel(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 */
bool vault_aux_treasure(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 */
bool vault_aux_clone(int r_idx) {
 ...
 */
bool vault_aux_symbol(int r_idx) {
 ...
 */
bool vault_aux_orc_ogre(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 */
bool vault_aux_troll(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 */
bool vault_aux_giant(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
...
bool vault_aux_lesser_giant(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 */
bool vault_aux_dragon(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
 ...
 */
bool vault_aux_demon(int r_idx) {
	monster_race *r_ptr = &r_info[r_idx];
...
s_printf("deep-lava (%d,%d,%d)\n", wpos->wx, wpos->wy, wpos->wz);
	...
	for (i = 0; i < 1000; i++) {
		...
		if (get_skill(SKILL_DRUID) &&
		    (p_ptr->druid_extra & CLASS_FLOOD_LEVEL)) {
			if (i < 333) floor_type[i] = FEAT_DEEP_WATER;
			else floor_type[i] = FEAT_SHAL_WATER;
		}
		else
#endif	/* 0 */
		{
...
//s_printf("FORGET\n");
	...
	if (dun_lev > 2 && (town ||
		...
		} // else -> generate fixed dungeon town at defined depth
#endif
...
/*..(already covered?) */
	...
	if ((rand_int(1000) < cfg.dungeon_shop_chance) || netherrealm_level ||
						...
						if ((cs_ptr = AddCS(csbm_ptr, CS_SHOP))) {
							...
							if (!netherrealm_level) {
								if (build_special_store == 1) {
									...
									} else cs_ptr->sc.omni = cfg.dungeon_shop_type;
							...
							} else cs_ptr->sc.omni = STORE_BTSUPPLY;
		...
		if (!netherrealm_level) {
			if (build_special_store == 1) dungeon_store_timer = 0;
			else dungeon_store2_timer = 0;
		}
	}
}
...
void alloc_dungeon_level(struct worldpos *wpos) {
	...
	if (wpos->wz) {
		...
		default: //compromise for wilderness-travel ambient sfx hacking
			w_ptr->ambient_sfx_timer = 30 + rand_int(10); //should be > than time required for travelling across 1 wilderness sector
