...
#if 0 /* currently all done by quest_check_goal_target() master function */
...
static void quest_check_goal_target_wpos(int Ind, monster_type *m_ptr, object_type *o_ptr) {
	quest_info *q_ptr;
	player_type *p_ptr = Players[Ind];
	int i, j, q_idx, stage;
...
/* Check if player completed a goal restricted to a target wpos and specific x,y.
   Those can be either kill-goals or item-retrieve-goals.
   The monster slain or item retrieved must therefore be given to this function for examination. */
static void quest_check_goal_target_xy(int Ind, monster_type *m_ptr, object_type *o_ptr) {
	quest_info *q_ptr;
	player_type *p_ptr = Players[Ind];
	int i, j, q_idx, stage;
