...
*/
	...
	if (!p_ptr->rogue_heavyarmor && p_ptr->dual_wield && p_ptr->dual_mode)
		...
		/* Alternative values (+0...+15, but increase slower at the beginning and faster towards 50):
		   #1: "5300/(160-DWSKILL)-33" , #2: 23400/(300-DWSKILL)-78 */
			...
			if (!wild_info[wpos.wy][wpos.wx].dungeon) {
				...
				if (!ge->extra[5]) {
					...
					add_dungeon(&wpos, 1, 50, DF1_UNLISTED | DF1_NO_RECALL, DF2_IRON | DF2_NO_EXIT_MASK |
					    ...
					    DF3_NO_SIMPLE_STORES | DF3_NO_DUNGEON_BONUS | DF3_EXP_20 | DF3_NOT_WATERY | DF3_VMANY_MONSTERS, FALSE, 0, 0, 0, 0);
				} else {
