...
//BAD!(recursion)			set_tim_wraith(Ind, 30000);
	...
	if (r_ptr->flags3 & RF3_AURA_COLD) { p_ptr->sh_cold = p_ptr->sh_cold_fix = TRUE; csheet_boni->cb[10] |= CB11_AELEC; }
...
Exceptions are rare, like Ent, who as a being of wood is suspectible to fire. (C. Blue) */
	...
	if (r_ptr->flags7 & RF7_CAN_FLY) {
		p_ptr->levitate = TRUE; csheet_boni->cb[6] |= CB7_RRLEV;
		p_ptr->feather_fall = TRUE; csheet_boni->cb[4] |= CB5_RFALL;
	}
...
//BAD!(recursion)				set_tim_wraith(Ind, 30000);
		...
		if (f4 & (TR4_LEVITATE)) {
			p_ptr->levitate = TRUE; csheet_boni[i-INVEN_WIELD].cb[6] |= CB7_RRLEV;
...
//			if (elapsed_turns - (ge->announcement_time * cfg.fps) - 600 == 600) { /* after 10 minutes of deathmatch phase */
	...
	case GE_DUNGEON_KEEPER:
		...
		case 0: { /* prepare level, gather everyone, begin */
			...
			sector00flags2 = LF2_NO_RUN | LF2_NO_TELE | LF2_NO_DETECT | LF2_NO_ESP | LF2_NO_SPEED | LF2_NO_RES_HEAL | LF2_FAIR_TERRAIN_DAM;
