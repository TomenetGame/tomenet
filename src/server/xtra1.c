...
void announce_global_event(int ge_id) {
	...
	if (time_left >= 120) msg_broadcast_format(0, "\374\377W[%s (%d) starts in %d minutes]", ge->title, ge_id + 1, time_left / 60);
...
#endif
#if 0 /* random, but guarantee # of open doors, iterating over the 'rooms'. */
	/* problem: a room can get too many (up to 4) doors generated by its adjacent rooms, from each side */
			for (x = 2; x < MAX_WID - 1; x += 4)
			for (y = 2; y < MAX_HGT - 1; y += 4) {
...
#endif
#if 1 /* generate max # of doors, then remove some randomly */
