...
struct object_type {
	...
	s16b find_special;		/* Found in a chest (sval*1000+lev), from digging (10000+feat), disarming (11000+trap_idx), from artifact creation (12000), from poly-ring forging (12001),
...
//UNUSED but set in do_cmd_steal and do_life_scroll
	...
	byte temp_misc_3; //0x01: for combosets, to skip set-gain messages once: on login, 0x02: skip recalculating stat_xxx[] array during calc_boni()
	byte lifetime_flags;
...
//not needed!	int colour_phase; /* for mimics mimicking multi-coloured stuff */
	...
	char warning_tss, warning_stdwager, warning_res, warning_doubleres, warning_AMC, warning_empty; //AMC is kinda redundant with standard enter-AMC-info messages
