...
void init_stuff(void) {
  ...
  #ifdef USE_SDL3
	if (chdir(SDL3_GAME_PATH) == -1) plog_fmt("init_stuff: chdir(\"%s\") failed", SDL3_GAME_PATH);
  #else
	if (argv0) {
	...
	if (!strlen(path)) {
  ...
  #ifdef USE_SDL3
		if (!tail) {
			/* Compose default: SDL3_GAME_PATH + DEFAULT_PATH */
			int n = snprintf(path, sizeof(path), "%s%s", SDL3_GAME_PATH, DEFAULT_PATH);
		...
		if (tail != path) strcpy(path, tail ? tail : DEFAULT_PATH);
	}

	/* Hack -- Add a path separator (only if needed) */
  #ifdef USE_SDL3
	if (!suffix(path, SDL3_PATH_SEP)) strcat(path, SDL3_PATH_SEP);
  #else
...
void artifact_lore_aux(int aidx, int alidx, char paste_lines[18][MSG_LEN], bool to_chat) {
	...
	while (0 == my_fgets(fff, buf, 1024)) {
		...
		/* Glitch fix: Replace \37.- by \377U so we don't fall back on charactermode-specific colour */
...
static void Input_loop(void) {
	...
	/* ---- TODO: A little order glitch, that we workaround here - should fix this in a cleaner manner probably: ----     - C. Blue
	        The visual modules (init_x11() / init_gcu() / init_windows() / init_sdl3()) are loaded BEFORE client_init() is called.
...
/* Don't flush events in SDL3. Flushing it will fail to register some key presses. */
#ifndef USE_SDL3
		/* Flush input (now!) */
		flush_now();
#endif
...
static void quit_hook(cptr s) {
	...
	/* Save the live window layout before writing preferences. */
	if (strcmp(ANGBAND_SYS, "gcu")) all_term_data_to_term_prefs();
#endif
...
void client_init(char *argv1, bool skip) {
...
#if defined(USE_X11) || defined(USE_GCU) || defined(USE_SDL3)
	...
	{
/* SDL3 socklib uses pseudo-fd indexes, so Unix fd syscalls are valid only without USE_SDL3. */
#if defined(USE_X11) || (defined(USE_GCU) && !defined(USE_SDL3))
		...
		/* Native linux X11 client behaviour is (at the time of writing) impossible using OS independent code or SDL3 libraries. */
		/* This behaves like Windows client and extracts and stores IP addres to `ip_iface` variable (using OS independent code and SDL3 libraries). */
		/* On my setup, the native x11 client sets ip_iface to 'lo', windows client to '192.168.88.3' and SDL3 clients to '127.0.0.1'. */
		int count = 0;
		NET_Address **addrs = NET_GetLocalAddresses(&count);
		const char *chosen_ip = NULL;

		/* Primary path: pick the best local IPv4 from SDL_net interface list. */
		if (!addrs) {
			c_msg_format("NET_GetLocalAddresses failed: %s\n", SDL_GetError());
		} else {
			for (int i = 0; i < count; ++i) {
				const char *ip = NET_GetAddressString(addrs[i]);

				if (!ip || strchr(ip, ':')) continue;
				if (!strcmp(ip, "0.0.0.0") || !strcmp(ip, "255.255.255.255")) continue;
				if (!prefix(ip, "127.")) {
		...
		if (chosen_ip) {
			strncpy(ip_iface, chosen_ip, MAX_CHARS - 1);
...
again:
 ...
 #endif
#elif defined(USE_SDL3)
		// Getting MAC address is not possible using only SDL3. Generate a deterministic fake MAC.
		// Prefix stays constant ("F4K"), suffix derived from a local SDL3-friendly fingerprint.
		{
			MD5_CTX ctx;
			...
			c_msg_print("");
			MD5Init(&ctx);
			MD5Update(&ctx, (const unsigned char *)"TomenetSDL3Fingerprint", sizeof("TomenetSDL3Fingerprint") - 1);
			MD5Update(&ctx, (const unsigned char *)"\0", 1);

			MD5Update(&ctx, (const unsigned char *)SDL3_USER_PATH, strlen(SDL3_USER_PATH));
			MD5Update(&ctx, (const unsigned char *)"\0", 1);

			len = snprintf(tmp, sizeof(tmp), "%d", SDL_GetNumLogicalCPUCores());
	...
	/* Note: The SDL3 client returns always 0 as port number. But it doesn't matter, cause the server throws this information away and probes the port by itself. */
 ...
 #if defined(USE_X11) || defined(WINDOWS) || defined(USE_SDL3)
