...
struct term_data {
	...
	XImage *fgmask_sub[MAX_SUBFONTS];
 ...
 #ifdef GRAPHICS_BG_MASK
	XImage *bgmask_sub[MAX_SUBFONTS];
 #endif
#endif
};
...
static void free_graphics(term_data *td) {
	...
	if (td->fgmask) {
		XDestroyImage(td->fgmask);
		td->fgmask = NULL;
	}
 #ifdef GRAPHICS_BG_MASK
	if (td->bgmask) {
		XDestroyImage(td->bgmask);
		td->bgmask = NULL;
	}
	...
	for (i = 0; i < MAX_SUBFONTS; i++) {
        ...
        if (td->fgmask_sub[i]) {
			XDestroyImage(td->fgmask_sub[i]);
			td->fgmask_sub[i] = NULL;
		}
 #ifdef GRAPHICS_BG_MASK
		if (td->bgmask_sub[i]) {
			XDestroyImage(td->bgmask_sub[i]);
...
static errr Term_pict_x11(int x, int y, byte a, char32_t c) {
	...
	XImage *preparedTile = prepareTile(tiles, fgmask, td->fnt->wid, td->fnt->hgt, c);
...
Pixell xInterpolationBilinear(XImage *originalImage, float originalX, float originalY, rectangle tile_boundaries)
{
	...
	int originalLoopY = floor(originalY);
	float fractionOfX = originalX - originalLoopX;
	float fractionOfY = originalY - originalLoopY;

	// top left
	coordinates topLeftPixelCoordinates;
	topLeftPixelCoordinates = confineCoordinatesToRectangle(floor(originalX), floor(originalY), tile_boundaries);
	...
	topRightPixelCoordinates = confineCoordinatesToRectangle(ceil(originalX), floor(originalY), tile_boundaries);
	...
	bottomLeftPixelCoordinates = confineCoordinatesToRectangle(floor(originalX), ceil(originalY), tile_boundaries);
...
static void term_data_init_graphics(int index, term_data *td) {
	...
	if (use_graphics) {
		...
		} else td->tiles = ResizeImage(Metadpy->dpy, graphics_image, graphics_tile_wid, graphics_tile_hgt, td->fnt->wid, td->fnt->hgt);
		...
		} else td->fgmask = ResizeImage(Metadpy->dpy, graphics_fgmask, graphics_tile_wid, graphics_tile_hgt, td->fnt->wid, td->fnt->hgt);
		...
		} else td->bgmask = ResizeImage(Metadpy->dpy, graphics_bgmask, graphics_tile_wid, graphics_tile_hgt, td->fnt->wid, td->fnt->hgt);
 #endif
		...
		for (i = 0; i < MAX_SUBFONTS; i++) {
			...
			} else td->tiles_sub[i] = ResizeImage(Metadpy->dpy, graphics_image_sub[i], graphics_tile_wid, graphics_tile_hgt, td->fnt->wid, td->fnt->hgt);
			...
			} else td->fgmask_sub[i] = ResizeImage(Metadpy->dpy, graphics_fgmask_sub[i], graphics_tile_wid, graphics_tile_hgt, td->fnt->wid, td->fnt->hgt);
			...
			} else td->bgmask_sub[i] = ResizeImage(Metadpy->dpy, graphics_bgmask_sub[i], graphics_tile_wid, graphics_tile_hgt, td->fnt->wid, td->fnt->hgt);
 #endif
