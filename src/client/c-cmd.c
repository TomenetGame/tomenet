 ...
 */
static int sdl3_fileman(const char *p) {
	...
	if (!is_abs) {
		fprintf(stderr, "Warning: sdl3_fileman: File path \"%s\" is not absolute, trying to fix it, but please rewrite the code to call with absolute path.\n", p);
		strnfmt(buf, sizeof(buf), "%s%s", SDL3_GAME_PATH, p);
		p = buf;
	}
	strnfmt(url, sizeof(url), "file://%s", p);
	if (!SDL_OpenURL(url)) {
		c_msg_format("Opening file \"%s\" failed with error: %s", p, SDL_GetError());
		return -1;
	}
	return 0;
}
 ...
 * Returns 0 on success, or -1 on error. The error is written as a chat message.
 */
static int sdl3_urlman(const char *p) {
	if (!SDL_OpenURL(p)) {
		c_msg_format("Opening url \"%s\" failed with error: %s", p, SDL_GetError());
		return -1;
	}
	return 0;
}
 #define FILEMAN(p) (res = sdl3_fileman(p))
 #define URLMAN(p) (res = sdl3_urlman(p))
#endif
	...
	while (i != ESCAPE) {
		if (redraw) {
			...
			if (strlen(mangrc_filename) <= 35)
				...
				i = 35;
				while (i && mangrc_filename[(int)i] != SDL3_PATH_SEP[0]) i--;
				if (i != 35) i++;
				/* ..but don't accept unreasonable lenght */
				if (i < 20) {
					/* Cut without regard, ouch */
