...
int using_subinven = -1, using_subinven_size = -1, using_subinven_item = -1;
...
bool sdl3_window_decorations = FALSE;
int sdl3_graphics_image_force_outline = -1;
#endif
...
int weather_wind = 0; /* current gust of wind if any (1 strong west, 2 strong east, 3 west, 4 east) */
...
byte panel_map_a[MAX_WINDOW_WID][MAX_WINDOW_HGT] = { 0 };
char32_t panel_map_c[MAX_WINDOW_WID][MAX_WINDOW_HGT] = { 0 };
#ifdef GRAPHICS_BG_MASK
byte panel_map_a_back[MAX_WINDOW_WID][MAX_WINDOW_HGT] = { 0 };
char32_t panel_map_c_back[MAX_WINDOW_WID][MAX_WINDOW_HGT] = { 0 };
#endif
...
int cfg_a, cfg_m, cfg_s, cfg_w, cfg_va, cfg_vm, cfg_vs, cfg_vw;
...
char monster_list_name[MAX_R_IDX][MAX_CHARS], monster_list_symbol[MAX_R_IDX][2]; //note: [0] is the attr (just byte), [1] is the char. Todo maybe: Separate attr and char.
...
/* For artifact lore */
char artifact_list_name[MAX_A_IDX][MAX_CHARS]; //note: format of this is: 1st byte \377, 2nd byte attr, 3rd byte char, then "Lnn " and the actual artifact name
...
char artifact_list_activation[MAX_A_IDX][MAX_CHARS];
/* For artifact lore */
char kind_list_name[MAX_K_IDX][MAX_CHARS];
...
char kind_list_dizline[MAX_K_IDX][MSG_LEN] = { 0 };
...
char macro_trigger_exclusive[MACROKEY_LEN]; //hack for 'd'eleting a macro, to auto-restore the original key
