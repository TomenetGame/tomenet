...
void display_store_action(void) {
	...
	// print all existing store actions
	for (i = 0; i < MAX_STORE_ACTIONS; i++) {
...
static void store_sell(void) {
	...
	if (store_num == STORE_HOME || store_num == STORE_HOME_DUN) {
		if (!c_get_item(&item, "Drop what? ", (USE_EQUIP | USE_INVEN | UNPREFER_SUBINVEN)))
			return;
	} else if (store_num == STORE_MATHOM_HOUSE) {
		if (!c_get_item(&item, "Donate what? ", (USE_EQUIP | USE_INVEN | UNPREFER_SUBINVEN)))
			return;
	} else {
		if (!c_get_item(&item, "Sell what? ", (USE_EQUIP | USE_INVEN | UNPREFER_SUBINVEN)))
			return;
	}
...
void store_do_command(int num, bool one) {
	...
	if (c_store.flags[num] & BACT_F_INVENTORY) {
		...
		if (c_store.flags[num] & BACT_F_STAR_ID) {
			...
			if (!c_get_item(&item, "Which item? ", get_item_mode | UNPREFER_SUBINVEN)) return;
			...
			get_item_mode |= USE_EXTRA;
			if (!c_get_item(&item, "Which armour? ", get_item_mode | UNPREFER_SUBINVEN)) return;
			...
			get_item_mode |= USE_EXTRA;
			if (!c_get_item(&item, "Which weapon? ", get_item_mode | UNPREFER_SUBINVEN)) return;
			...
			get_item_mode |= USE_EXTRA;
			if (!c_get_item(&item, "What ammunition? ", get_item_mode | UNPREFER_SUBINVEN)) return;
			...
			get_item_mode |= USE_EXTRA;
			if (!c_get_item(&item, "Which ranged weapon? ", get_item_mode | UNPREFER_SUBINVEN)) return;
			...
			get_item_mode |= USE_EXTRA;
			if (!c_get_item(&item, "Which melee weapon? ", get_item_mode | UNPREFER_SUBINVEN)) return;
		...
		} else if (!c_get_item(&item, "Which item? ", get_item_mode | UNPREFER_SUBINVEN)) return;
	}
