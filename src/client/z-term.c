...
/*
 * Place cursor at (x,y), and clear the next "n" chars.
 * Unlike Term_erase() this re-clears even already 'clear' grids.
 * Added this to clear special image visuals (graphics mode) which do not imprint char/attr
 * onto the screen and hence are 'ghost' visuals that Term_erase() cannot know of. - C. Blue
 */
errr Term_erase_force(int x, int y, int n) {
	int w = Term->wid;
	/* int h = Term->hgt; */
