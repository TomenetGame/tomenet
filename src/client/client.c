...
#ifdef USE_SDL3
static void sdl3_normalize_path_separators(char *path) {
	char *c;

	if (!path || SDL3_PATH_SEP[0] != '\\') return;

	for (c = path; *c; c++) {
		if (*c == '/') *c = SDL3_PATH_SEP[0];
	}
}

static char *sdl3_normalize_root_path(cptr path, cptr label) {
	char *resolved_path;
	...
	if (resolved_path[strlen(resolved_path) - 1] == SDL3_PATH_SEP[0])
		strnfmt(normalized_path, sizeof(normalized_path), "%s", resolved_path);
	else
		strnfmt(normalized_path, sizeof(normalized_path), "%s%s", resolved_path, SDL3_PATH_SEP);
...
static bool read_mangrc(cptr filename) {
	...
	if (!strlen(filename)) {
		...
		if (snprintf(mangrc_filename, MAX_PATH_LENGTH, "%s%s", SDL3_USER_PATH, filename) > MAX_PATH_LENGTH) {
...
#ifdef USE_SDL3
		snprintf(default_config_path, 1024, "%s%s", SDL3_GAME_PATH, filename);
	...
	while (!feof(config)) {
...
#ifdef USE_GRAPHICS
 #if defined(USE_SDL3) && defined(GRAPHICS_BG_MASK)
		...
		if (!strncmp(buf, "graphicsForceOutline", 20)) {
			...
			p = strtok(NULL, "\t\n");
			if (p && *p) {
				sdl3_graphics_image_force_outline = atoi(p);
				if (sdl3_graphics_image_force_outline < -1) sdl3_graphics_image_force_outline = -1;
				if (sdl3_graphics_image_force_outline > SDL3_FORCE_OUTLINE_MAX_RADIUS) sdl3_graphics_image_force_outline = SDL3_FORCE_OUTLINE_MAX_RADIUS;
			}
			else sdl3_graphics_image_force_outline = -1;
		}
 #endif
 #if defined(USE_X11) || defined(USE_SDL3)
		...
		if (!strncmp(buf, "windowDecorations", 17)) {
			...
			p = strtok(buf, " \t\n");
			p = strtok(NULL, "\t\n");
			if (p) sdl3_window_decorations = (atoi(p) != 0);
		}
#endif
...
bool write_mangrc(bool creds_only, bool update_creds, bool audiopacks_only) {
...
#if defined(USE_X11) || defined(WINDOWS) || defined(USE_SDL3) /* Ie not on pure USE_GCU/ANDROID -- (redundant though: WINDOWS doesn't use this function as it doesn't use .rc files) */
	int i;
			...
			while (!feof(config)) {
				...
				if (audiopacks_only) {
					...
					if (!creds_only) {
						...
						if (!strcmp(ANGBAND_SYS, "x11") || !strcmp(ANGBAND_SYS, "sdl3")) {
							bool win = FALSE;
...
#endif /* defined(USE_X11) || defined(USE_SDL3) */
					}
...
#ifdef USE_SDL3
			/* Don't do this in terminal mode ('-c') */
			if (!strcmp(ANGBAND_SYS, "sdl3") && explicit_save) {
 #ifdef USE_GRAPHICS
				if (!found_graphic_resize_type && !audiopacks_only) {
 ...
 #endif
 #if defined(USE_SDL3) && defined(GRAPHICS_BG_MASK)
			fputs(format("graphicsForceOutline\t\t%d\n", sdl3_graphics_image_force_outline), config2);
 #endif
#endif
			fputs("\n", config2);
//#ifdef USE_SOUND
...
//#endif
			...
			fputs("\n", config2);
#ifdef USE_SDL3
			fputs(format("windowDecorations\t\t%d\n", sdl3_window_decorations ? 1 : 0), config2);
			fputs("\n", config2);
#endif
...
int main(int argc, char **argv) {
...
#ifdef USE_SDL3
	{
		...
		SDL3_PATH_SEP[0] = base_path[strlen(base_path) - 1];
		SDL3_PATH_SEP[1] = '\0';

		SDL3_GAME_PATH = sdl3_normalize_root_path(base_path, "SDL_GetBasePath");
		if (!SDL3_GAME_PATH) return(-1);
	}

	const char *user_path = getenv( "TOMENET_SDL3_USER_PATH");
	if (user_path && user_path[0]) {
		...
		sdl3_normalize_path_separators(user_path_buf);
		...
		if (check_dir2(normalized_user_path) == FALSE) {
			...
			if (!sdl3_ensure_parent_dirs(format("%s%s.", normalized_user_path, SDL3_PATH_SEP))) {
				fprintf(stderr, "main: The \"%s\" directory found in %s env does not exist and can't be created\n", normalized_user_path,  "TOMENET_SDL3_USER_PATH");
				return(-1);
			}
		}
		SDL3_USER_PATH = sdl3_normalize_root_path(normalized_user_path, "TOMENET_SDL3_USER_PATH");
		if (!SDL3_USER_PATH) return(-1);
		if (check_dir2(normalized_user_path) == FALSE) {
		...
		char *pref_path = SDL_GetPrefPath(SDL3_ORG_NAME, SDL3_GAME_NAME);

		if (!pref_path) {
			fprintf(stderr, "main: SDL_GetPrefPath Error: %s\n", SDL_GetError());
			return(-1);
		}
		SDL3_USER_PATH = sdl3_normalize_root_path(pref_path, "SDL_GetPrefPath");
		SDL_free(pref_path);
		if (!SDL3_USER_PATH) return(-1);
	}
#endif
	...
	for (i = 1; argv && (i < argc); i++) {
		...
		/* Pull 'real name' */
		case 'n':
			if (argv[i][2]) { /* Hack -- allow space after '-n' */
...
#endif
#ifdef USE_SDL3
		case 's': ignore_scaling = true; break;
#endif
		}
...
#endif
#ifdef USE_SDL3
	if (ignore_scaling) logprint("Display scaling is ignored.\n");
#endif
	...
	if (!force_cui) {
		...
		if (!done) {
			...
			if (done && use_graphics) {
				handle_process_graphics_file();
				/* Clear the temporary graphics initialization screen shown by init_sdl3(). */
				Term_clear();
				Term_fresh();
				Term_flush();
			}
