...
#if defined(USE_X11) || defined(WINDOWS) || defined(USE_SDL3) /* Ie not on pure USE_GCU/ANDROID */
 #define TEST_RAWPICT
#endif
//#endif
...
int Net_init(int fd) {
	...
	wbuf.sock = sock;
/* SDL3_net lack setters for delay and buffer sizes. */
#ifndef USE_SDL3
...
#ifdef USE_GRAPHICS
 #ifdef TEST_RAWPICT
	if (!c_cfg.ascii_items && use_graphics && Term->rawpict_hook) {
...
static void display_fruit(int row, int col, int fruit) {
	...
	bool use_gfx_fruits = /* perhaps display both, first ascii fruit and then gfx fruit on top of it, so ascii fruit will still be there if screen gets refreshed? */
	    ...
	    FALSE;
 #else
	    !c_cfg.ascii_items && use_graphics && Term->rawpict_hook &&
