...
static errr Infowin_init(int x, int y, int w, int h, unsigned int b, Pixel b_color, Pixel bg_color) {
	...
	SDL_Window *window = SDL_CreateWindow("", w, h, flags);
	...
	Uint64 windowFlags = SDL_GetWindowFlags(window);
	Infowin->shown = !(windowFlags & SDL_WINDOW_HIDDEN);
...
static errr Infowin_resize(int w, int h) {
	/* Execute the request */
	if (!SDL_SetWindowSize(Infowin->window, w, h)) return(1);
	Infowin->wb = w;
	Infowin->hb = h;
...
static errr Infowin_wipe(void) {
	...
	uint32_t bc = SDL_MapRGBA(SDL_GetPixelFormatDetails(Infowin->surface->format), NULL, r, g, b, a);
	...
	SDL_FillSurfaceRect(Infowin->surface, &(SDL_Rect){0, 0, Infowin->wb, Infowin->bh}, bc);
	SDL_FillSurfaceRect(Infowin->surface, &(SDL_Rect){0, Infowin->bh + Infowin->hd, Infowin->wb, (Infowin->hb - Infowin->hd - Infowin->bh)}, bc);
	SDL_FillSurfaceRect(Infowin->surface, &(SDL_Rect){0, Infowin->bh + 1, Infowin->bw, Infowin->hd}, bc);
	SDL_FillSurfaceRect(Infowin->surface, &(SDL_Rect){Infowin->bw + Infowin->wd, Infowin->bh + 1, Infowin->wb - Infowin->wd - Infowin->bw, Infowin->hd}, bc);
#else
	SDL_FillSurfaceRect(Infowin->surface, &(SDL_Rect){0, 0, Infowin->wb, SDL3_DEFAULT_BORDER_WIDTH}, bc);
	SDL_FillSurfaceRect(Infowin->surface, &(SDL_Rect){0, Infowin->hb - SDL3_DEFAULT_BORDER_WIDTH, Infowin->wb, Infowin->hb}, bc);
	SDL_FillSurfaceRect(Infowin->surface, &(SDL_Rect){0, SDL3_DEFAULT_BORDER_WIDTH + 1, SDL3_DEFAULT_BORDER_WIDTH, Infowin->hb - SDL3_DEFAULT_BORDER_WIDTH - 1}, bc);
	SDL_FillSurfaceRect(Infowin->surface, &(SDL_Rect){Infowin->wb - SDL3_DEFAULT_BORDER_WIDTH, SDL3_DEFAULT_BORDER_WIDTH + 1, Infowin->wb, Infowin->hb - SDL3_DEFAULT_BORDER_WIDTH - 1}, bc);
#endif
	/* Success */
	return(0);
}
...
void terminal_window_real_coords_sdl3(int term_idx, int *ret_x, int *ret_y);
void resize_main_window_sdl3(int cols, int rows);
void resize_window_sdl3(int term_idx, int cols, int rows);
 ...
 */
static void resize_window_sdl3_timers_handle(void) {
	uint32_t now = 0;
	int t_idx;
	...
	for (t_idx = 0; t_idx < ANGBAND_TERM_MAX; t_idx++) {
		...
		if (td->resize_timer != 0) {
			...
			if (now >= td->resize_timer) {
				...
				resize_window_sdl3(t_idx, (td->win->wb - (2 * SDL3_DEFAULT_BORDER_WIDTH)) / td->fnt->wid, (td->win->hb - (2 * SDL3_DEFAULT_BORDER_WIDTH)) / td->fnt->hgt);
...
static int CheckEvent(bool wait) {
	...
	resize_window_sdl3_timers_handle();
...
static int Term_win_update(int flush, int sync, int discard) {
	...
	if (flush) {
		...
		if (td == &term_main) {
			...
			uint32_t bc = SDL_MapRGBA(SDL_GetPixelFormatDetails(td->win->surface->format), NULL, r, g, b, a);
			/* Draw top, bottom, left and right border. */
 #ifdef SDL3_FILL_BORDERS
			SDL_FillSurfaceRect(td->win->surface, &(SDL_Rect){0, 0, td->win->wb, td->win->bh}, bc);
			SDL_FillSurfaceRect(td->win->surface, &(SDL_Rect){0, td->win->bh + td->win->hd, td->win->wb, (td->win->hb - td->win->hd - td->win->bh)}, bc);
			SDL_FillSurfaceRect(td->win->surface, &(SDL_Rect){0, td->win->bh + 1, td->win->bw, td->win->hd}, bc);
			SDL_FillSurfaceRect(td->win->surface, &(SDL_Rect){td->win->bw + td->win->wd, td->win->bh + 1, td->win->wb - td->win->wd - td->win->bw, td->win->hd}, bc);
 #else
			SDL_FillSurfaceRect(td->win->surface, &(SDL_Rect){0, 0, td->win->wb, SDL3_DEFAULT_BORDER_WIDTH}, bc);
			SDL_FillSurfaceRect(td->win->surface, &(SDL_Rect){0, td->win->hb - SDL3_DEFAULT_BORDER_WIDTH, td->win->wb, td->win->hb}, bc);
			SDL_FillSurfaceRect(td->win->surface, &(SDL_Rect){0, SDL3_DEFAULT_BORDER_WIDTH + 1, SDL3_DEFAULT_BORDER_WIDTH, td->win->hb - SDL3_DEFAULT_BORDER_WIDTH - 1}, bc);
			SDL_FillSurfaceRect(td->win->surface, &(SDL_Rect){td->win->wb - SDL3_DEFAULT_BORDER_WIDTH, SDL3_DEFAULT_BORDER_WIDTH + 1, td->win->wb, td->win->hb - SDL3_DEFAULT_BORDER_WIDTH - 1}, bc);
 #endif
		}
#endif
...
static errr term_data_init(int index, term_data *td, bool fixed, cptr name, cptr font) {
	...
	if (Infofnt_init(font) == -1) {
		...
		if (Infofnt_init(sdl3_terms_font_default[index]) == -1) {
	...
	Infowin_init(topx, topy, wid_border, hgt_border, SDL3_DEFAULT_BORDER_WIDTH, color_default_b, color_default_bg);
...
static void term_force_font(int term_idx, cptr fnt_name) {
	...
	if ((old_fnt_wid != td->fnt->wid) || (old_fnt_hgt != td->fnt->hgt)) {
		...
		resize_window_sdl3(term_idx, td->t.wid, td->t.hgt);
...
void resize_window_sdl3(int term_idx, int cols, int rows) {
	...
	if (Infowin->wb != wid_border || Infowin->hb != hgt_border) {
		...
		if ((windowFlags & SDL_WINDOW_MAXIMIZED) == false) {
			if (Infowin_resize(wid_border, hgt_border)) {
				fprintf(stderr, "Error resizing window: %s\n", SDL_GetError());
				Infowin_set(iwin);
				Term_activate(t);
				return;
			}
	...
	/* Window resizing may be asynchronous. Wait for it before replacing the window-owned surface, which SDL invalidates when the size changes. */
...
void resize_main_window_sdl3(int cols, int rows) {
	resize_window_sdl3(0, cols, rows);
}
