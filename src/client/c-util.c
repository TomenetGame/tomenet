...
/* Parse a macro stage filename of form "<base>-FS<digit>.prf". */
static bool macroset_parse_stage_name(const char *name, const char *base, char *base_out, size_t base_out_len, int *stage) {
	...
	if (name[name_len - 8] != '-' || name[name_len - 7] != 'F' || name[name_len - 6] != 'S') return FALSE;
	...
	if (base) {
		...
		if (name_len != base_len + 8) return FALSE;
...
static bool macroset_scan_for_set(int set_idx, char **entries, size_t entries_count, bool *entries_used) {
	...
	for (i = 0; i < entries_count; i++) {
		...
		fileset[set_idx].stage_file_exists[stage] = TRUE;
		...
		while (++m < macro__num) {
			...
			*cfile = 0;
			...
			if (f == k) {
				...
				fileset[k].style_freesw = style_freesw;
					...
					case mw_fileset: {
						...
						while (TRUE) {
								...
								for (k = 0; k < MACROFILESETS_MAX; k++)
									if (k < filesets_found) {
										...
										Term_putstr(xoffset2 - 1, l++, -1, fileset_selected == k ? TERM_VIOLET : TERM_UMBER,
										    ...
										    (fileset[k].style_cyclic && fileset[k].style_freesw) ? "Cyc+Fr" : (fileset[k].style_cyclic ? "Cyclic" : (fileset[k].style_freesw ? "FreeSw" : "------")),
							...
							case 'I': //init new fileset (implies initialization+activation of a 1st stage too)
								...
								} else if (fileset[f].style_freesw) Term_putstr(15, l++, -1, TERM_L_GREEN, "Switching method: \377sFreeSw");
							...
							case 'm': //modify switching method
								...
								style_freesw = fileset[fileset_selected].style_freesw;
...
static bool read_audio_pack_info(cptr dir, cptr config_file, char name[MAX_CHARS], char author[MAX_CHARS], char diz[MAX_CHARS * 3], char version[MAX_CHARS]) {
	...
	while (!feof(fff)) {
		...
		*cval = 0;
		...
		if (!strcmp(ckey, "description")) {
			strnfmt(diz, MAX_CHARS * 3, "%s", cval);
			continue;
		}
...
void audio_pack_selector(void) {
	...
	cur_sp = cur_mp = cur_sy = cur_my = 0;
	for (k = 0; k < soundpacks; k++) {
		read_audio_pack_info(sp_dir[k], "sound.cfg", sp_name[k], sp_author[k], sp_diz[k], sp_version[k]);
	...
	for (k = 0; k < musicpacks; k++) {
		read_audio_pack_info(mp_dir[k], "music.cfg", mp_name[k], mp_author[k], mp_diz[k], mp_version[k]);
