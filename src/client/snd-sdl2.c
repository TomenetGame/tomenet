...
static bool init_pack_paths(void) {
	char path[2048];
#ifndef SEGFAULT_HACK
	char *tmp;
	...
	if (!check_dir2(ANGBAND_USER_DIR_XTRA_SOUND)) {
		if (MKDIR(ANGBAND_USER_DIR_XTRA_SOUND) != 0 && errno != EEXIST) {
			plog("Failed to create user sound pack path.");
			return(FALSE);
		}
	}
	...
	if (!check_dir2(ANGBAND_USER_DIR_XTRA_MUSIC)) {
		if (MKDIR(ANGBAND_USER_DIR_XTRA_MUSIC) != 0 && errno != EEXIST) {
			plog("Failed to create user music pack path.");
			return(FALSE);
		}
	}
#else
	if (!ANGBAND_DIR_XTRA_SOUND[0]) {
		path_build(path, sizeof(path), ANGBAND_DIR_XTRA, cfg_soundpackfolder);
		strcpy(ANGBAND_DIR_XTRA_SOUND, path);
	}

	if (!ANGBAND_USER_DIR_XTRA_SOUND[0]) {
		path_build(path, sizeof(path), ANGBAND_USER_DIR_XTRA, cfg_soundpackfolder);
	...
	if (!ANGBAND_DIR_XTRA_MUSIC[0]) {
		path_build(path, sizeof(path), ANGBAND_DIR_XTRA, cfg_musicpackfolder);
		strcpy(ANGBAND_DIR_XTRA_MUSIC, path);
	}

	if (!ANGBAND_USER_DIR_XTRA_MUSIC[0]) {
		path_build(path, sizeof(path), ANGBAND_USER_DIR_XTRA, cfg_musicpackfolder);
...
static void log_music_started(int music_event, int music_song, const char *verb);
...
typedef struct {
	...
	bool force_disabled;		/* disabled by system - do not persist to user config */
	bool config;
...
static bool sample_event_is_runtime_disabled(int idx) {
	return(samples[idx].disabled || samples[idx].force_disabled);
}

static bool song_event_is_runtime_disabled(int idx) {
	return(songs[idx].disabled || songs[idx].force_disabled);
}

static runtime_disable_state clear_runtime_disable_state(bool *disabled, bool *force_disabled) {
	runtime_disable_state state;

	state.disabled = *disabled;
	state.force_disabled = *force_disabled;
	*disabled = FALSE;
...
static void restore_runtime_disable_state(bool *disabled, bool *force_disabled, runtime_disable_state state) {
	*disabled = state.disabled;
	*force_disabled = state.force_disabled;
}
...
static void invalidate_jukebox_restore_music(void) {
	jukebox_org = -1;
	jukebox_org_song = -1;
	jukebox_org_repeat = 0;
	jukebox_org_vol = 100;
	jukebox_paused = FALSE;
}

static void remember_jukebox_restore_music(void) {
...
static void refresh_jukebox_restore_music(void) {
	remember_jukebox_restore_music();

	/* If a fade-out already committed to a follow-up track, restore that target instead. */
	if (Mix_FadingMusic() == MIX_FADING_OUT) {
		jukebox_org = music_next;
		jukebox_org_song = (music_next == -1 ? -1 : music_next_song);
		jukebox_org_repeat = 0;
		if (music_next >= 0 && music_next < MUSIC_MAX &&
		    music_next_song >= 0 && music_next_song < songs[music_next].num &&
		    !songs[music_next].initial[music_next_song]) {
			jukebox_org_repeat = (c_cfg.shuffle_music || c_cfg.play_all ? 0 : -1);
			if (songs[music_next].num == 1) jukebox_org_repeat = -1;
		}
	}
}
...
static void close_audio(void) {
	...
	/* Prevent music-finished callbacks from running while audio/UI state is being torn down. */
	...
	curmus_timepos = -1;
	Mix_HaltMusic();
	music_cur = music_cur_song = -1;
	music_next = music_next_song = -1;
...
/* Check whether a raw config line contains an event entry, optionally behind "<n>|". */
static bool is_audio_config_entry(char *line) {
	char *p = trim_leading_whitespace(line);
	char *pipe, *eq;
	...
	if (isdigit((unsigned char)*p)) {
		pipe = strchr(p, '|');
		eq = strchr(p, '=');
		if (pipe && eq && eq > pipe) p = trim_leading_whitespace(pipe + 1);
	}
...
static bool add_sound_sample(int event, const char *token, int line_no) {
	...
	path_build(path, sizeof(path), ANGBAND_USER_DIR_XTRA_SOUND, token);
	errno = 0;
	if (!my_fexists(path) && !sdl2_paths_same(ANGBAND_USER_DIR_XTRA_SOUND, ANGBAND_DIR_XTRA_SOUND)) {
...
static bool add_music_song(int event, const char *token, bool initial_flag, int line_no) {
	...
	path_build(path, sizeof(path), ANGBAND_USER_DIR_XTRA_MUSIC, token);
	errno = 0;
	if (!my_fexists(path) && !sdl2_paths_same(ANGBAND_USER_DIR_XTRA_MUSIC, ANGBAND_DIR_XTRA_MUSIC)) {
		if (errno == EMFILE || errno == ENFILE) {
...
static bool audio_and_config_init(void) {
	...
	/* Initialize sound and music pack paths. */
	if (!init_pack_paths()) return(audio_init_fail());
...
extern bool sound_bell(void) {
	...
	if (sample_event_is_runtime_disabled(bell_sound_idx)) return(TRUE); /* claim that it 'succeeded' */
...
extern bool sound_page(void) {
	...
	if (sample_event_is_runtime_disabled(page_sound_idx)) return(TRUE); /* claim that it 'succeeded' */
...
extern bool sound_warning(void) {
	...
	if (sample_event_is_runtime_disabled(warning_sound_idx)) return(TRUE); /* claim that it 'succeeded' */
...
static bool play_sound(int event, int type, int vol, s32b player_id, int dist_x, int dist_y) {
	...
	if (sample_event_is_runtime_disabled(event)) return(TRUE); /* claim that it 'succeeded' */
...
static void play_sound_weather(int event) {
	...
	if (sample_event_is_runtime_disabled(event)) {
...
void jukebox_update_songlength(void) {
	...
	if (music_cur == -1 || music_cur_song == -1
	    || !Mix_PlayingMusic()
	    || !songs[music_cur].wavs[music_cur_song]) {
		curmus_song_dur = 0;
		return; //paranoia
	}
...
static bool play_music(int event) {
	...
	if (song_event_is_runtime_disabled(event)) {
		music_next = event;
		music_next_song = 0;

		/* Disabled music events still transition to silence and update bookkeeping. */
		if (Mix_PlayingMusic()) {
			if (Mix_FadingMusic() != MIX_FADING_OUT) Mix_FadeOutMusic(500);
		} else fadein_next_music();
		return(TRUE);
	}
	...
	if (music_cur != -1 && music_cur_song != -1 &&
	    !song_event_is_runtime_disabled(music_next) && songs[music_next].num /* Check there are samples for this event */
...
static void log_music_started(int music_event, int music_song, const char *verb) {
	const char *path, *c, *c2, *lua_name;
	char out_val[MAX_CHARS];

	if (music_event < 0 || music_song < 0) return;
	path = songs[music_event].paths[music_song];
	if (!path) return;

	c = path + strlen(path);
	c2 = c;

	sprintf(out_val, "return get_music_name(%d)", music_event);
	lua_name = string_exec_lua(0, out_val);

	while (c > path && *(c - 1) != SDL2_PATH_SEP[0]) c--;
	while (*c2 != '.' && c2 > c) c2--;
	if (!lua_name) lua_name = "";

	if (c2 == c) c_msg_format_multiline("\377WMusic <%s> %s. (%s)", c, verb, lua_name);
	else c_msg_format_multiline("\377WMusic <%.*s> %s. (%s)", (int)(c2 - c), c, verb, lua_name);
}

static bool restore_jukebox_org_music(bool override_log) {
	Mix_Music *wave = NULL;
	int vols = 100;
	...
	/* Paranoia: Special hack values should just restore silence. */
	if (jukebox_org < -1) jukebox_org = -1;

	/* Restore the remembered "silent" state instead of replaying anything. */
	if (jukebox_org == -1 || song_event_is_runtime_disabled(jukebox_org)) {
		play_music_instantly(-2, FALSE);
	...
	/* If the remembered subsong became invalid somehow, fall back to normal event restore. */
	if (jukebox_org_song < 0 || jukebox_org_song >= songs[jukebox_org].num) {
		if (jukebox_org_vol == 100) return(play_music(jukebox_org));
		return(play_music_vol(jukebox_org, jukebox_org_vol));
	}
	...
	if (!wave && !(wave = load_song(music_cur, music_cur_song))) {
		plog(format("SDL music load failed (%d, %d) [restore].", music_cur, music_cur_song));
		puts(format("SDL music load failed (%d, %d) [restore].", music_cur, music_cur_song));
		return(FALSE);
	}
	/* Apply user-defined custom volume modifier. */
	...
	music_vol = jukebox_org_vol;
	if (music_vol != 100) Mix_VolumeMusic(CALC_MIX_VOLUME(cfg_audio_music, (cfg_audio_music_volume * evlt[(int)music_vol]) / MIX_MAX_VOLUME, vols));
	else Mix_VolumeMusic(CALC_MIX_VOLUME(cfg_audio_music, cfg_audio_music_volume, vols));

	Mix_FadeInMusic(wave, music_cur_repeat, 1000);

	/* Also log songs while auto-play is active outside the jukebox. */
	if ((!jukebox_screen || jukebox_play_all) && c_cfg.log_music && override_log) log_music_started(music_cur, music_cur_song, "started");
	return(TRUE);
}
...
static void fadein_next_music(void) {
	...
	if (jukebox_play_all) {
		...
		if (music_cur != -1 && music_cur_song == songs[music_cur].num - 1) {
			for (j = 0; j < MUSIC_MAX; j++) {
				...
				if (!songs[d].config || song_event_is_runtime_disabled(d)) continue;
				break;
			}
			if (j == MUSIC_MAX) {
				...
				if (c_cfg.log_music) c_msg_format("\377WMusic auto-play finished.");
				set_mixing();
	...
	if (
		...
		if (song_event_is_runtime_disabled(music_cur)) return;
		...
		if (!jukebox_screen && c_cfg.log_music) log_music_started(music_cur, music_cur_song, "started");
		return;
	}
	...
	if (song_event_is_runtime_disabled(music_next)) {
		/* Disabled events are silent, but keep event identity for jukebox re-enable. */
...
//#endif
	...
	if (!jukebox_screen && c_cfg.log_music) {
		log_music_started(music_cur, music_cur_song, was_resumed ? "resumed" : "started");
#else
	if (!jukebox_screen && c_cfg.log_music) {
		log_music_started(music_cur, music_cur_song, "started");
#endif
	}
}
...
errr re_init_sound_sdl(void) {
	...
	music_next = -1;
	music_next_song = -1;
	invalidate_jukebox_restore_music();
...
static Mix_Chunk* load_sample(int idx, int subidx) {
	...
	if (samples[idx].wavs[subidx]) {
		samples[idx].force_disabled = FALSE;
#ifdef DEBUG_SOUND
...
static Mix_Music* load_song(int idx, int subidx) {
	...
	if (songs[idx].wavs[subidx]) {
		songs[idx].force_disabled = FALSE;
#ifdef DEBUG_SOUND
...
static void clear_audio_options_screen(bool *screen_flag) {
	*screen_flag = FALSE;
	topline_icky = FALSE;
}

/* Extract an event name from a raw config line, handling optional ";", "<n>|", and whitespace. */
static bool extract_audio_event_name(char *line, char *event_name, size_t event_name_len) {
	char buffer[4096];
	char *key, *pipe, *eq;

	if (!event_name_len) return(FALSE);

	strncpy(buffer, line, sizeof(buffer));
	buffer[sizeof(buffer) - 1] = '\0';

	key = trim_leading_whitespace(buffer);
	if (*key == ';') key = trim_leading_whitespace(key + 1);
	if (!*key) return(FALSE);

	if (isdigit((unsigned char)*key)) {
		pipe = strchr(key, '|');
		eq = strchr(key, '=');
		if (pipe && eq && eq > pipe) key = trim_leading_whitespace(pipe + 1);
	}
	...
	*eq = '\0';
	...
	strncpy(event_name, key, event_name_len);
	event_name[event_name_len - 1] = '\0';
	return(TRUE);
}

static int lookup_saved_audio_event(audio_config_kind kind, const char *event_name) {
	if (kind == AUDIO_CONFIG_SOUND) return(exec_lua(0, format("return get_sound_index(\"%s\")", event_name)));
	return(exec_lua(0, format("return get_music_index(\"%s\")", event_name)));
}

static bool audio_event_has_config(audio_config_kind kind, int idx) {
	if (kind == AUDIO_CONFIG_SOUND) return(samples[idx].config);
	return(songs[idx].config);
}

static bool audio_event_is_disabled(audio_config_kind kind, int idx) {
	if (kind == AUDIO_CONFIG_SOUND) return(samples[idx].disabled);
	return(songs[idx].disabled);
}

static int audio_event_volume(audio_config_kind kind, int idx) {
	if (kind == AUDIO_CONFIG_SOUND) return(samples[idx].volume);
	return(songs[idx].volume);
}

/* Rewrite user audio cfg by copying the active config and applying disabled/reset state. */
static bool save_custom_audio_config(audio_config_kind kind, const char *config_name, const char *config_dir,
	const char *user_dir, bool reset_all, char *user_path, size_t user_path_len, bool *screen_flag) {
	int event_idx;
	bool was_commented;
	size_t leading_ws_len;
	char buf[1024], buf2[1024], out_val[4096], out_val2[4096], *p, *line, *content, evname[4096];
	FILE *fff, *fff2;
	const char *audio_label = (kind == AUDIO_CONFIG_SOUND) ? "sound" : "music";

	path_build(user_path, user_path_len, user_dir, config_name);

	/* Open current configuration for reading. */
	/* Note: For SDL2 clients, my_fopen already handles reading from user dir with game dir fallback. */
	path_build(buf, sizeof(buf), config_dir, config_name);
	fff = my_fopen(buf, "r");
	if (!fff) {
		c_msg_format("Error: Cannot read any %s config file ['%s', '%s'].", audio_label, user_path, buf);
		clear_audio_options_screen(screen_flag);
		return(FALSE);
	}

	/* Write changes to a file in user-writable temp dir. */
	path_build(buf2, sizeof(buf2), os_temp_path, config_name);
	fff2 = my_fopen(buf2, "w");
	if (!fff2) {
		c_msg_format("Error: Cannot write to temporary %s config file '%s'.", audio_label, buf2);
		clear_audio_options_screen(screen_flag);
		fclose(fff);
		return(FALSE);
	}
	while (TRUE) {
		if (!fgets(out_val, sizeof(out_val), fff)) {
			if (ferror(fff)) c_msg_format("Error: Failed to read from %s config file '%s'.", audio_label, my_fexists(user_path) ? user_path : buf);
			break;
		}

		line = out_val;
		content = trim_leading_whitespace(line);
		leading_ws_len = content - line;
		p = content;
		was_commented = (*p == ';');
		if (was_commented) p = trim_leading_whitespace(p + 1);

		if (reset_all) {
			/* Reset must persist even if pack metadata is currently unavailable. */
			if (!is_audio_config_entry(p)) fputs(out_val, fff2);
			else {
				snprintf(out_val2, sizeof(out_val2), "%.*s%s", (int)leading_ws_len, line, p);
				fputs(out_val2, fff2);
			}
			continue;
		}

		if (!extract_audio_event_name(p, evname, sizeof(evname))) {
			fputs(out_val, fff2);
			continue;
		}

		event_idx = lookup_saved_audio_event(kind, evname);
		if (event_idx == -1 || !audio_event_has_config(kind, event_idx)) {
			/* 'empty' event (no filenames specified), just copy it over same as misc lines */
			fputs(out_val, fff2);
			continue;
		}

		if (audio_event_is_disabled(kind, event_idx)) {
			snprintf(out_val2, sizeof(out_val2), "%.*s;%s", (int)leading_ws_len, line, p);
		} else snprintf(out_val2, sizeof(out_val2), "%.*s%s", (int)leading_ws_len, line, p);
	...
	/* Backup previous user configuration, if any. */
	if (my_fexists(user_path)) my_fcopy(user_path, format("%s.bak", user_path));

	/* Replace user configuration with the above created temp file. */
	if (my_fcopy(buf2, user_path) != 0) {
		c_msg_format("Error: Cannot write %s config file '%s'.", audio_label, user_path);
		remove(buf2);
		clear_audio_options_screen(screen_flag);
		return(FALSE);
	}
...
/* Save per-event volume overrides from the current user cfg into the dedicated volume file. */
static bool save_custom_audio_volume(audio_config_kind kind, const char *user_config_path, const char *volume_dir,
	const char *user_dir, const char *volume_name, bool reset_all, bool *screen_flag) {
	int event_idx, volume;
	char buf[1024], out_val[4096], out_val2[4096], *p, evname[4096];
	FILE *fff, *fff2;
	const char *audio_label = (kind == AUDIO_CONFIG_SOUND) ? "sound" : "music";

	/* Open the volume configuration for writing. */
	/* Note: For SDL2 clients, my_fopen already handles writing to writable user storage. */
	path_build(buf, sizeof(buf), volume_dir, volume_name);
	fff2 = my_fopen(buf, "w");
	if (!fff2) {
		path_build(buf, sizeof(buf), user_dir, volume_name);
		c_msg_format("Error: Cannot write to %s volume config file '%s'.", audio_label, buf);
		clear_audio_options_screen(screen_flag);
		return(FALSE);
	}
	if (reset_all) {
	...
	/* Read the freshly written user config and emit only non-default volumes. */
	fff = my_fopen(user_config_path, "r");
	if (!fff) {
		c_msg_format("Error: Cannot read %s config file '%s'.", audio_label, user_config_path);
		clear_audio_options_screen(screen_flag);
		fclose(fff2);
		return(FALSE);
	}
	while (TRUE) {
		if (!fgets(out_val, sizeof(out_val), fff)) {
			if (ferror(fff)) c_msg_format("Error: Failed to read from %s config file '%s'.", audio_label, user_config_path);
			break;
		}
		...
		if (!extract_audio_event_name(p, evname, sizeof(evname))) continue;

		event_idx = lookup_saved_audio_event(kind, evname);
		if (event_idx == -1 || !audio_event_has_config(kind, event_idx)) continue;

		volume = audio_event_volume(kind, event_idx);
		if (!volume) continue;
...
static void save_custom_sound(bool reset_all) {
	char buf_user[1024];

	if (!save_custom_audio_config(AUDIO_CONFIG_SOUND, "sound.cfg", ANGBAND_DIR_XTRA_SOUND,
		ANGBAND_USER_DIR_XTRA_SOUND, reset_all, buf_user, sizeof(buf_user), &jukebox_sfx_screen)) return;
	if (!save_custom_audio_volume(AUDIO_CONFIG_SOUND, buf_user, ANGBAND_DIR_XTRA_SOUND,
		ANGBAND_USER_DIR_XTRA_SOUND, "TomeNET-soundvol.cfg", reset_all, &jukebox_sfx_screen)) return;
}
...
void do_cmd_options_sfx_sdl(bool reset) {
	...
	bool go = TRUE;
	runtime_disable_state state;
	char buf[1024], buf_user[1024], out_val[4096];
	...
	if (reset) {
		/* A pack switch already reloaded the new pack-specific overrides, so do not wipe them. */
	...
	/* Build the filename in user folder. */
	path_build(buf_user, 1024, ANGBAND_USER_DIR_XTRA_SOUND, "sound.cfg");
	/* Check if the file exists */
	if (!my_fexists(buf_user) && !sdl2_paths_same(ANGBAND_USER_DIR_XTRA_SOUND, ANGBAND_DIR_XTRA_SOUND)) {
		/* Build the filename in game folder. */
		path_build(buf, 1024, ANGBAND_DIR_XTRA_SOUND, "sound.cfg");
		if (!my_fexists(buf)) {
			c_msg_format("\377oError: Cannot read sound config files ['%s', '%s'].", buf_user, buf);
			return;
		}
	}
	...
	while (go) {
		...
		for (i = y - list_size ; i <= y + list_size; i++) {
			...
			if (sample_event_is_runtime_disabled(j)) {
				a = TERM_L_DARK;
				a2 = TERM_UMBER;
			} else {
				a = TERM_WHITE;
		...
		case KTRL('F'):
			...
			interact_audio();
			jukebox_sfx_screen = TRUE;
			cfg_audio_master_org = cfg_audio_master;
			cfg_audio_sound_org = cfg_audio_sound;
			cfg_audio_music_org = cfg_audio_music;
			cfg_audio_weather_org = cfg_audio_weather;
			if (!audio_sfx) {
				c_msg_print("\377yNo sound effects available.");
		...
		case 't': //case ' ':
			if (sample_event_is_runtime_disabled(j_sel)) {
				samples[j_sel].disabled = FALSE;
				samples[j_sel].force_disabled = FALSE;
			...
			break;
		case 'y':
			if (!samples[j_sel].disabled && !samples[j_sel].force_disabled) break;
			samples[j_sel].disabled = FALSE;
			samples[j_sel].force_disabled = FALSE;
			if (j_sel == weather_current) {
				weather_current = -1; //allow restarting it
		...
		case '\r':
			...
			state = clear_runtime_disable_state(&samples[j_sel].disabled, &samples[j_sel].force_disabled);
#ifdef ENABLE_SHIFT_SPECIALKEYS
...
#endif
			sound(j_sel, SFX_TYPE_MISC, 100, 0, 0, 0);
			restore_runtime_disable_state(&samples[j_sel].disabled, &samples[j_sel].force_disabled, state);
...
static int jukebox_music_visible_index(int event) {
	int j, d = -1;
...
static void save_custom_music(bool reset_all) {
	char buf_user[1024];

	if (!save_custom_audio_config(AUDIO_CONFIG_MUSIC, "music.cfg", ANGBAND_DIR_XTRA_MUSIC,
		ANGBAND_USER_DIR_XTRA_MUSIC, reset_all, buf_user, sizeof(buf_user), &jukebox_screen)) return;
	if (!save_custom_audio_volume(AUDIO_CONFIG_MUSIC, buf_user, ANGBAND_DIR_XTRA_MUSIC,
		ANGBAND_USER_DIR_XTRA_MUSIC, "TomeNET-musicvol.cfg", reset_all, &jukebox_screen)) return;
}
...
void do_cmd_options_mus_sdl(bool reset) {
	...
	/* Check if the file exists */
	if (!my_fexists(buf_user) && !sdl2_paths_same(ANGBAND_USER_DIR_XTRA_MUSIC, ANGBAND_DIR_XTRA_MUSIC)) {
		/* Build the filename in game folder. */
		path_build(buf, 1024, ANGBAND_DIR_XTRA_MUSIC, "music.cfg");
		if (!my_fexists(buf)) {
			c_msg_format("\377oError: Cannot read music config files ['%s', '%s'].", buf_user, buf);
			return;
		}
	}

	if (jukebox_playing == -1) { /* <- check for jukebox_play_all hack that allows leaving the jukebox yet continuing to play-all */
		refresh_jukebox_restore_music();
	}
	jukebox_screen = TRUE;
	...
	while (go) {
		Term_putstr(0, 0, -1, TERM_WHITE, format(" \377ydir\377w/\377yp\377w/\377ySPC\377w/\377yg\377w/\377yG\377w/\377y#\377w/\377ys\377w/\377yS\377w/'\377y/\377w', \377yc\377w cur, \377yt\377w/\377yy\377w/\377yn\377w toggle/on/off, \377yv\377w/\377y+\377w/\377y-\377w vol, \377yCTRL+%c\377w mixer, \377yESC \377wsave", c_cfg.rogue_like_commands ? 'F' : 'U'));
#ifdef ENABLE_SHIFT_SPECIALKEYS
		if (strcmp(ANGBAND_SYS, "gcu"))
			Term_putstr(0, 1, -1, TERM_WHITE, format(" \377y[SHIFT+]RETURN\377w/\377ya\377w/\377yu\377w/\377ye\377w [200%% vol] play/all/shuffle/event, \377yLEFT\377w/\377yRIGHT\377w rw/ff %ds, \377yP\377w %s",
			    MUSIC_SKIP, jukebox_paused ? (jukebox_play_all ? "\3777resume" : "\3774resume") : "pause "));
		else /* GCU cannot query shiftkey states easily, see macro triggers too (eg cannot distinguish between ENTER and SHIFT+ENTER on GCU..) */
#endif
			Term_putstr(0, 1, -1, TERM_WHITE, format(" \377yRETURN\377w/\377ya\377w/\377yA\377w/\377yu\377w/\377yU\377w/\377ye\377w/\377yE\377w play/all/shuffle/event [at 200%%], \377yLEFT\377w/\377yRIGHT\377w rw/ff %ds, \377yP\377w pause", MUSIC_SKIP));
		...
		for (i = y - list_size ; i <= y + list_size ; i++) {
			...
			/* set colour depending on enabled/disabled state */
			//todo - c_cfg.use_color D: yadayada
			if (song_event_is_runtime_disabled(j)) {
				a = TERM_L_DARK;
				a2 = TERM_UMBER;
			} else {
				a = TERM_WHITE;
		...
		if (ch == -1) {
			...
			if (cur_event == -1) {
				cur_event = music_cur; /* Happens if play-all just finished and we resume normal operation of the actual game song playing before. */
				d = jukebox_music_visible_index(cur_event);
			}
			if (d != -1) {
				y = d;
				j_sel = cur_event;
		...
		case KTRL('F'):
			...
			cfg_audio_music_org = cfg_audio_music;
			cfg_audio_weather_org = cfg_audio_weather;
			if (jukebox_playing == -1) refresh_jukebox_restore_music();
			if (!audio_music) {
				c_msg_print("\377yNo music available.");
		...
		case 't': //case ' ':
			if (song_event_is_runtime_disabled(j_sel)) {
				songs[j_sel].disabled = FALSE;
				songs[j_sel].force_disabled = FALSE;
				if (music_cur == j_sel) {
		...
		case 'y':
			if (!songs[j_sel].disabled && !songs[j_sel].force_disabled) break;
			songs[j_sel].disabled = FALSE;
			songs[j_sel].force_disabled = FALSE;
			if (music_cur == j_sel) {
				int jbo = jukebox_org; //the play_music() call below will modify jukebox_org wrongly, so we have to keep it here and restore it afterwards
		...
		case '\r':
...
#ifdef JUKEBOX_INSTANT_PLAY
			state = clear_runtime_disable_state(&songs[j_sel].disabled, &songs[j_sel].force_disabled);
			...
			restore_runtime_disable_state(&songs[j_sel].disabled, &songs[j_sel].force_disabled, state);
#else
			if (j_sel == music_cur) break;
			if (song_event_is_runtime_disabled(j_sel)) break;
		...
		case 'U':
			...
			if (jukebox_play_all) {
				...
				restore_jukebox_org_music(FALSE);
				if (jukebox_static200vol) Mix_VolumeMusic(CALC_MIX_VOLUME(cfg_audio_music, cfg_audio_music_volume, 200));
		...
		case 'w': /* Skip to next subsong */
			...
			if (jukebox_playing != -1 && !jukebox_play_all) { //jukebox_subonly_play_all) {
				...
				state = clear_runtime_disable_state(&songs[d].disabled, &songs[d].force_disabled);
				play_music_instantly(d, FALSE);
				restore_runtime_disable_state(&songs[d].disabled, &songs[d].force_disabled, state);
